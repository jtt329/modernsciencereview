...
assert.match(routesSource, /const REVIEW_JOB_AUTO_RECOVERY = process\.env\.REVIEW_JOB_AUTO_RECOVERY !== "false"/);
...
assert.match(routesSource, /if \(!REVIEW_JOB_AUTO_RECOVERY\) return/);
assert.match(routesSource, /Durable review job auto-recovery disabled/);
...
assert.match(submissionFormSource, /Server restarted during review; item will retry/);
