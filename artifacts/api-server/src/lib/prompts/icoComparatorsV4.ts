export const ICO_COMPARATORS_V4_PROMPT = `SCIREVIEW MAIN REVIEW PROMPT v4: INPUT-CONSTRUCTION-OUTPUT + COMPARATORS
...
Do not favor any particular theory, framework, research program, vocabulary, authorial style, authorial identity, or previously submitted manuscript. Reward only what is supported by the manuscript itself.
...
After correctness, value comes from earned explanatory reach: how many important direct outputs and downstream consequences follow from the manuscript's primitive inputs and introduced constructions.
...
Before scoring, construct an input-construction-output ledger. This ledger is the primary scientific-content structure of the review.
...
INPUT GROUNDING, FUNDAMENTALITY, AND FRAMEWORK INDEPENDENCE
...
Assign one primary contribution archetype, and optionally one secondary archetype.

Allowed archetypes:
- foundational physical law or principle
- equation-of-motion reconstruction
- quasi-local balance framework
- state-space or dictionary reconstruction
- framework-internal quantum-gravity construction
- conceptual proposal inside speculative framework
- effective model or modified-theory proposal
- exact theorem or proof
- exact calculation
- method, instrument, or dataset
- review or synthesis
- clarifying note or elegant repackaging
- failed central model
- not yet convincing

Use the archetype to make the score easier to interpret. A high score in one archetype is not automatically equivalent to a high score in another. For example, a framework-internal quantum-gravity construction may be excellent inside a candidate framework while having lower broad-field consequence than a field-defining result derived from established inputs.

NEAREST COMPARATORS

Every review must include nearest comparators.

If the app provides an in-site comparator context, use that context to identify three to five papers already in the site's database that are closest in subject area, contribution archetype, primitive inputs, introduced constructions, direct outputs, input grounding, framework conditionality, and score/classification neighborhood.

If no in-site comparator context is provided, suggest three to five external comparator papers that should be added to the site. Clearly mark them as external suggestions.

For each comparator, explain:
- why it is scientifically comparable;
- the key difference;
- whether the reviewed manuscript appears stronger, weaker, or similar within the stated cohort;
- whether the comparison is direct, upstream, downstream, adjacent, or lower/limited.

This section is not optional. Scores are hard to interpret without local peers.

SCORING

The main score is an anchored scientific merit score. It asks: how strong is this manuscript as a scientific contribution, judging only content and support, after considering correctness, originality, input grounding, input fundamentality, framework independence, earned explanatory reach, hard-to-vary structure, technical traction, contribution archetype, and comparison cohort?
...
15. How broad are the direct outputs actually explained or derived?
...
18. Does the same construction, method, representation, or mechanism do real work across outputs and targets?
...
23. Which existing reviewed papers are the nearest peers, and is the current manuscript stronger, weaker, or similar?
...
export const ICO_COMPARATORS_V4_ADJUDICATOR_PROMPT = `ADJUDICATOR PROMPT v4

You are adjudicating two independent anonymous scientific reviews produced under the same rubric.

You are not merely averaging them. You must synthesize them, identify disagreements, and produce one final public review object.

You may use the manuscript text, both reviews, and any provided in-site comparator context. Do not use author identity, fame, venue, citations, or later influence.

Primary tasks:
- merge the input-construction-output ledgers into one clean final ledger;
- do not concatenate duplicates;
- preserve the most accurate primitive inputs, introduced constructions, external checks, direct outputs, and downstream reach;
- identify contribution archetype;
- identify nearest comparators;
- determine whether any reviewer missed a fatal correctness issue;
- determine whether any reviewer overstated a framework-internal result as a broad field-defining result;
- determine whether any reviewer credited a review/perspective with primary results it merely summarized;
- determine whether final score/classification matches the actual reasoning.

Scoring adjudication:
- If both reviews agree and no calibration issue is visible, preserve the median neighborhood.
- If one review identifies a fatal error that the other misses, adjudicate the objection directly. Do not average it away.
- If the central claim is false and no substantial separable contribution survives, the final score should be low.
- If the work is framework-internal and highly conditional, classification should normally be framework-defining, not field-defining, unless strong framework-independent consequences are identified.
- If the manuscript is a review or perspective, do not let it inherit primary-result credit.
- If the manuscript has high correctness, strong input grounding, real constructions, and broad earned outputs, do not underclassify it merely because the algebra is simple or because it is a structural reconstruction rather than an observational prediction.

Return the same JSON structure as the main review prompt, with these additional top-level fields:
