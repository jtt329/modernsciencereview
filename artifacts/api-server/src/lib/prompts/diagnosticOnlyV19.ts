...
2. constructionStrengthScore (Construction Strength): correctness,
   ...
   compression or resolves real ambiguity. Do not equate polish with
   strength: a first-of-its-kind construction that survives basic
   consistency checks earns origination credit even at heuristic rigor; a
   rigorous formalization of an existing concept is valuable but is not
   the same achievement as originating it.
...
For each central output, classify the firmness of its referent — the
thing the output is about:
F1. Directly measured phenomena or data.
F2. Established-theory regimes with strong indirect evidence
    (astrophysical black holes, FRW cosmology, de Sitter-like
    acceleration as measured).
F3. Plausible but unobserved constructs of established theory
    (horizon interiors, Hawking radiation of stellar-mass holes).
F4. Constructs internal to untested frameworks (extremal or
    supersymmetric black holes, AdS spacetimes, higher dimensions,
    loop quantum geometry, string vacua).
Output Strength must reflect referent firmness: an exact result about
an F4 referent is a framework-internal achievement and must not score
as if it established a fact about nature. State the firmness class of
each central output in the rationale. A manuscript whose central
outputs are all F4 should not receive near-frontier Output Strength
...
Grade the firmness of inputs and frameworks by empirical confirmation
status alone: theories validated by direct experiment and observation
rank highest (necessary but not sufficient for a 10, which also requires
depth and generality); experimentally invalidated theories rank lowest;
everything between is placed by the actual state of evidence. Apply this
grading symmetrically to all research programs: the volume, popularity,
or prestige of a program's literature is not evidence and must not enter
the assessment. Within equally untested frameworks, a parameter-free
exact result outranks a parameter-tuned one, but both remain
framework-internal. Popularity within the theoretical literature is not
evidence; do not import its prestige hierarchy.
...
influence; prefer "the manuscript" over "the author." Never print C1-C5
or F1-F4 labels in scientificReview or any public-facing prose; describe
what they mean in words. Labels are allowed in structured fields only.
...
export const INTRINSIC_ADJUDICATOR_V19_ADDENDUM = String.raw`Adjudicator instructions
------------------------
...
export const INTRINSIC_ADJUDICATOR_V19_PROMPT = [
