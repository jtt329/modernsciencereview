...
export default function ReviewCard({ review, onLike, isLiked, isAdmin = false }: ReviewCardProps) {
  ...
  const currentSubscoreRationale =
    ...
    {};
  // "Why not 10" — a display-only line generated by the gated score-reduction
  // pass from the stored rationale. Never affects the score; absent until the
  // pass has run for this review.
  const scoreReductionReasons =
    selectedPass?.scoreReductionReasons ??
  ...
  const diagnosticCards = [
    {
      ...
      reductionReason: typeof scoreReductionReasons?.input === 'string' ? scoreReductionReasons.input : '',
    ...
    },
    {
      ...
      reductionReason: typeof scoreReductionReasons?.construction === 'string' ? scoreReductionReasons.construction : '',
    ...
    },
    {
      ...
      reductionReason: typeof scoreReductionReasons?.output === 'string' ? scoreReductionReasons.output : '',
              ...
              <div className="space-y-3">
                ...
                {activeDiagnosticCard && (
                  <div className="mb-4 rounded-xl border border-indigo-300/15 bg-indigo-300/[0.045] p-4 text-sm leading-relaxed text-slate-200">
                    ...
                    {activeDiagnosticCard.reductionReason && activeDiagnosticCard.value != null && activeDiagnosticCard.value < 10 && (
                      <p className="mb-2 rounded-lg border border-amber-300/20 bg-amber-300/[0.06] px-3 py-2 text-[13px] italic leading-relaxed text-amber-100/90">
                        {activeDiagnosticCard.reductionReason}
                      </p>
                    )}
