            ...
            <div className="flex flex-col items-end gap-1 shrink-0">
              ...
              {paper.modelName && (
                <span className="text-[9px] font-semibold text-slate-400 tracking-wide">
                  {paper.modelName.startsWith('gemini-3.5-flash') ? 'Gemini 3.5 Flash x2 + Adjudicator' : paper.modelName.startsWith('gemini') ? 'Gemini x2 + Adjudicator' : paper.modelName.startsWith('gpt') ? 'GPT-5.4 Pro' : paper.modelName}
                </span>
              )}
            </div>
          )}
